...
class PlayQueueItem(BaseModel):
    ...
    start_now_from_position: Annotated[
        Optional[StrictInt],
        Field(
    ...
    @field_validator("playlist_sort_by")
    def playlist_sort_by_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(["name", "artist", "date", "album"]):
            raise ValueError(
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if (
            self.playlist_sort_by is None
            and "playlist_sort_by" in self.model_fields_set
        ):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "index": obj.get("index"),
            "metadata": PlaybackContentMetadata.from_dict(obj["metadata"])
            if obj.get("metadata") is not None
            else None,
            "playlistSortBy": obj.get("playlistSortBy"),
            "provider": PlayQueueItemType.from_dict(obj["provider"])
