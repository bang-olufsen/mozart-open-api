...
class RoomCompensationInfo(BaseModel):
    ...
    last_result: Annotated[
        Optional[RoomCompensationResult], Field(alias="lastResult")
    ] = None
    last_run: Annotated[Optional[RoomCompensationState], Field(alias="lastRun")] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "debug": RoomCompensationDebug.from_dict(obj["debug"])
            if obj.get("debug") is not None
            else None,
            "enabled": RoomCompensationEnabled.from_dict(obj["enabled"])
            if obj.get("enabled") is not None
            else None,
            "lastResult": RoomCompensationResult.from_dict(obj["lastResult"])
            if obj.get("lastResult") is not None
            else None,
            "lastRun": RoomCompensationState.from_dict(obj["lastRun"])
            if obj.get("lastRun") is not None
            else None,
            "type": RoomCompensationType.from_dict(obj["type"])
            if obj.get("type") is not None
            else None,
            "version": RoomCompensationVersion.from_dict(obj["version"])
